...
// Waist Circumference Percentiles (Table 18 Data)
const WAIST_PERCENTILES: Record<'boys' | 'girls', Record<number, [number, number, number]>> = {
    ...
    girls: {
        ...
        10: [57.1, 68.6, 84.1], 11: [58.9, 69.2, 93.6], 12: [60.6, 73.9, 95.1], 13: [64.8, 73.7, 96.8],
        14: [67.5, 77.3, 94.4], 15: [68.5, 78.4, 101.4], 16: [67.9, 85.4, 102.2], 17: [69.0, 79.6, 99.5],
        18: [70.0, 77.2, 107.1]
    }
};
    ...
    return (
        <div className="mt-4 border border-gray-200 rounded-lg overflow-hidden text-xs">
            <div className="bg-gray-100 p-2 font-bold text-center border-b border-gray-200">
                Waist Circumference Percentiles (cm) - {gender === 'male' ? 'Boys' : 'Girls'} (2-18y)
            </div>
...
const ClientManager: React.FC<ClientManagerProps> = ({ initialClientId, onAnalyzeInKcal, onPlanMeals, onRunNFPE, autoOpenNew }) => {
  ...
  const detailedAge = formData.dob ? calculateDetailedAge(formData.dob) : null;
  const isPediatric = (detailedAge && detailedAge.years < 19) || (typeof formData.age === 'number' && formData.age < 19);
  const isUnder24Months = detailedAge ? (detailedAge.years * 12 + detailedAge.months) <= 24 : false;
  const isUnder36Months = detailedAge ? (detailedAge.years * 12 + detailedAge.months) <= 36 : false;
  ...
  useEffect(() => {
      if (formData.dob) {
          const calc = calculateDetailedAge(formData.dob);
          if (calc) setFormData(prev => ({ ...prev, age: calc.years }));
      }
  }, [formData.dob]);
                  ...
                  <div className="pt-4 border-t border-gray-100">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div>
                              ...
                              <input type="number" placeholder="Wt (kg)" value={formData.weight} onChange={e => setFormData({...formData, weight: e.target.value === '' ? '' : Number(e.target.value)})} className="w-full p-2 border rounded" />
                          </div>
                          <div>
                              <label className="block text-xs font-medium text-gray-500 mb-1">{isUnder24Months ? 'Length (cm)' : 'Height (cm)'}</label>
                              <input type="number" placeholder={isUnder24Months ? 'Length' : 'Height'} value={formData.height} onChange={e => setFormData({...formData, height: e.target.value === '' ? '' : Number(e.target.value)})} className="w-full p-2 border rounded" />
                          </div>
                          <div>
                              <label className="block text-xs font-medium text-gray-500 mb-1">Waist (cm)</label>
                              <input 
                                  type="number" 
                                  placeholder="Waist" 
                                  value={formData.waist} 
                                  onChange={e => setFormData({...formData, waist: e.target.value === '' ? '' : Number(e.target.value)})} 
                                  onFocus={() => isPediatric && setShowWaistChart(true)}
                                  className="w-full p-2 border rounded" 
                              />
                          </div>
                          <div>
                              <label className="block text-xs font-medium text-gray-500 mb-1">Hip (cm)</label>
                              <input type="number" placeholder="Hip" value={formData.hip} onChange={e => setFormData({...formData, hip: e.target.value === '' ? '' : Number(e.target.value)})} className="w-full p-2 border rounded" />
                          </div>
                          {isUnder36Months && (
                              <div className="col-span-2 md:col-span-1">
                                  <label className="block text-xs font-bold text-blue-600 mb-1">Head Circ (cm)</label>
                                  <input type="number" placeholder="HC (cm)" value={formData.head_circumference} onChange={e => setFormData({...formData, head_circumference: e.target.value === '' ? '' : Number(e.target.value)})} className="w-full p-2 border rounded border-blue-200 bg-blue-50" />
                              </div>
                          )}
                      </div>
                      ...
                      {showWaistChart && isPediatric && (
                          <div className="relative">
                              ...
                              <button type="button" onClick={() => setShowWaistChart(false)} className="absolute top-0 right-0 text-red-500 text-xs font-bold bg-white px-2 rounded border">Close Chart</button>
                          </div>
                      )}
                  </div>
              ...
              <div className="space-y-6">
                  ...
                  <form onSubmit={handleAddVisit} className="bg-white p-4 rounded-xl border border-blue-100 shadow-sm">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                          ...
                          <input type="number" placeholder={isUnder24Months ? 'Length (cm)' : 'Height (cm)'} value={newVisitData.height} onChange={e => setNewVisitData({...newVisitData, height: e.target.value === '' ? '' : Number(e.target.value)})} className="p-2 border rounded" />
                          {isUnder36Months && (
                              <input type="number" placeholder="Head Circ (cm)" value={newVisitData.head_circumference} onChange={e => setNewVisitData({...newVisitData, head_circumference: e.target.value === '' ? '' : Number(e.target.value)})} className="p-2 border rounded border-blue-200 bg-blue-50" />
                          )}
                          <input type="text" placeholder="Notes..." value={newVisitData.notes} onChange={e => setNewVisitData({...newVisitData, notes: e.target.value})} className="p-2 border rounded col-span-full md:col-span-2" />
                      </div>
                      ...
                      <table className="w-full text-sm text-left">
                          <thead className="bg-gray-50 text-gray-700 font-bold">
                              <tr>
                                  ...
                                  {isUnder36Months && <th className="p-3">HC</th>}
                          ...
                          <tbody>
                              {visits.map(v => (
                                  <tr key={v.id} className="border-t hover:bg-gray-50">
                                      ...
                                      {isUnder36Months && <td className="p-3">{v.head_circumference || '-'}</td>}
                              ...
                              {visits.length === 0 && <tr><td colSpan={6} className="p-4 text-center text-gray-400">No visits recorded.</td></tr>}
                          </tbody>
                      </table>
                  </div>
              </div>
